...
const isNumberValid = (inputString, {precision = 9, scale = 0}) => {
  if(parseInt(inputString) != NaN){
    if(inputString.length <= precision){
      if(!inputString.includes(".")){
        return true;
      }
      else if (inputString.split(".")[1].length <= scale && inputString.split(".")[1].length > 0 || inputString.split(".")[1] == undefined){
        return true;
      }
    }
  }
 ...
 *
 */
// https://stackoverflow.com/a/18234552
function add(x){
  return function(y){
    return x + y;
  };
